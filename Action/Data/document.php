...
function documentApplyMethod(Action &$action, $id,$method,&$returntype,&$out,&$doc=null) {
    ...
    switch($method) {
        ...
        case 'removeattribute':
            ...
            if (! $doc->error) {
                ...
                if ($method=="addattribute") {
                    $doc->addAttribute($aid,$label,$order,$type,$parent,$visibility,$inTitle,$inAbstract,
                    $needed,$link,$elink,$phpfile,$phpfunc,$constraint,$options);
                } elseif ($method=="removeattribute") {
                    $doc->removeAttribute($aid);
                } else {
                    $doc->modifyAttribute($aid,$label,$order,$type,$parent,$visibility,$inTitle,$inAbstract,
                    $needed,$link,$elink,$phpfile,$phpfunc,$constraint,$options);
                }
                $out=$doc->getDocument($onlyValues);
            } else $out->error=$doc->error;
        ...
        case 'getsearchcriteria':
            ...
            $out->error='';
            $out->operators=array();
            $alltype=array('integer','text','longtext','htmltext','docid','double',"ifile","array","file","image","enum","color","date","time","timestamp","money");
            foreach ($operators as $k=>$v) {
                ...
                foreach ($v["type"] as $type) {
                    ...
                    $out->operators[$type][]=array("operator"=>$k,"operand"=>$v["operand"],"label"=>$label,"tplLabel"=>$dynlabel);
                }
