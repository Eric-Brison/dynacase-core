    ...
    while (!feof($fdoc)) {
        ...
        switch ($data[0]) {
            ...
            case "DOC":
                ...
                $tcr[$nline] = csvAddDoc($dbaccess, $data, $dirid, $analyze, '', $policy, $tk, array() , $tcolorder[$fromid]);
            ...
            case "IATTR":
                ...
                if ($fi->isAffected()) {
                    ...
                    if ($fa) {
                        $oattri = new DocAttr($dbaccess, array(
                            $fiid,
                            strtolower($data[1])
                        ));
            ...
            case "PROFIL":
                ...
                else $pid = getIdFromName($dbaccess, trim($data[1]));
            ...
            case "ACCESS":
                if (ctype_digit(trim($data[1]))) $wid = trim($data[1]);
                    ...
                    $pid = getIdFromName($dbaccess, trim($data[1]));
            ...
            case "LDAPMAP":
                ...
                $aid = (trim($data[2]));
                $index = $data[5];
                $oa = new DocAttrLDAP($dbaccess, array(
                    $fid,
                    $aid,
                    $index
                ));
