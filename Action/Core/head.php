...
function head(&$action)
{
    ...
    if (isset($_GET["capp"])) $app->Set($_GET["capp"], $action->parent);
    else $app->Set($_GET["app"], $action->parent);
    ...
    if ($action->parent->exists("FREEGATE") && ($action->getParam("GATE_USEOLD") != "yes")) {
        $action->lay->setblockdata("FREEGATE", array(
            array(
                "zou"
            )
        ));
    } else {
        $action->lay->setblockdata("NOFREEGATE", array(
            array(
                "zou"
            )
        ));
