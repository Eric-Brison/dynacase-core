...
function gate_savegeo(&$action)
{
    ...
    $action->parent->param->Set("GATE_GEO", implode(",", $geometry) , PARAM_USER . $action->user->id, $action->parent->id);
    
    redirect($action, "CORE", "GATE_EDIT", $action->GetParam("CORE_STANDURL"));
}
?>
