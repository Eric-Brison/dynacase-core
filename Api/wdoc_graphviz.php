...
foreach ($doc->cycle as $k=>$v) {
  ...
  if ($label) { 
    ...
    if ($m2) {
      $line[]='"'.str_replace(" ","\\n",$m2.$k).'" [ label="'.$m2.'",fixedsize=false,shape=box,color="'."orange".'"];';
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue, labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$m2.$k));
      $e1=$m2.$k;
    ...
    if (count($mtrans)>0) {
      ...
      $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_TRANS_MTID".$v["t"]),
								       $doc->_array2val($mtrans),'_self',false));
      $timgt=' image="'.DEFAULT_PUBDIR.'/Images/tmail.png"';
      $line[]='"'.str_replace(" ","\\n",$ex).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=house,color="'."#a264d2".'"'.$timgt.' ];';
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue,labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$ex));
      $e1=$ex;      
    ...
    if (count($mt)>0) {
      ...
      $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_MTID".$v["e2"]),
							 $doc->_array2val($mt),'_self',false));
      $timgt=' image="'.DEFAULT_PUBDIR.'/Images/tmail.png"';
      $line[]='"'.str_replace(" ","\\n",$ex).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=house,color="'."#a264d2".'"'.$timgt.' ];';
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue, labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
    ...
    if (count($mt)>0) {
      $ex='tmf'.$k;
      $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_TMID".$v["e2"]),
							 $doc->_array2val($mt),'_self',false));
      $timgt=' image="'.DEFAULT_PUBDIR.'/Images/timer.png"';
      $line[]='"'.str_replace(" ","\\n",$ex).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=octagon,color="'."#64a2d2".'"'.$timgt.' ];';
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue,labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$ex));
      $e1=$ex;      
    ...
    if (count($ttrans)>0) {
      ...
      $tmlabel="tumer";
      $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_TRANS_MTID".$v["t"]),
								       $doc->_array2val($ttrans),'_self',false));
      $timgt=' image="'.DEFAULT_PUBDIR.'/Images/timer.png"';
      $line[]='"'.str_replace(" ","\\n",$ex).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=octagon,color="'."#64a2d2".'"'.$timgt.' ];';
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue,labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
    ...
    $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue, labelfontname=sans,label="%s" %s];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$e2),
		    _($v["t"]),$tmain);
  } else {
    ...
    $line[]=sprintf('"%s" -> "%s" [labelfontsize=6,color=darkblue %s,labelfontname=sans, label="%s"];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$e2),$tmain,($type=="activity")?_($v["t"]):"");
  }
}
...
if ((count($tm) > 0) || (count($mt) > 0)) {
  $line[]='"'.str_replace(" ","\\n",$e1).'" [shape = point,style=filled, width=0.3, fixedsize=true,fontname=sans,color=blue];';;
}
...
if (count($tm)>0) {
  ...
  $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_TMID".$doc->firstState),
								   $doc->_array2val($tm),'_self',false));
  $timgt=' image="'.DEFAULT_PUBDIR.'/Images/timer.png"';
  $line[]='"'.str_replace(" ","\\n",$e2).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=octagon,color="'."#64a2d2".'", fontsize='.$conditionfontsize.$timgt.' ];';
  $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue, labelfontname=sans];',
		  str_replace(" ","\\n",$e1),
		  str_replace(" ","\\n",$e2));
  $e1=$e2;      
...
if (count($mt)>0) {
  $e2='mtfirst';
  $tmlabel=str_replace(array("\n",'<BR>'),"\\n",$doc->getHtmlValue($doc->getAttribute($doc->attrPrefix."_MTID".$doc->firstState),
								   $doc->_array2val($mt),'_self',false));
  $timgt=' image="'.DEFAULT_PUBDIR.'/Images/tmail.png"';
  $line[]='"'.str_replace(" ","\\n",$e2).'" [ label="'.$tmlabel.'",fixedsize=false,style=bold,shape=house,color="'."#a264d2".'", fontsize='.$conditionfontsize.$timgt.' ];';
  $line[]=sprintf('"%s" -> "%s" [labelfontcolor="#555555",decorate=false, color=darkblue, labelfontname=sans];',
		  str_replace(" ","\\n",$e1),
		  str_replace(" ","\\n",$e2));
  $e1=$e2;      
