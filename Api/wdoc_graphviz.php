...
foreach ($doc->cycle as $k=>$v) {
  ...
  if (isset($doc->autonext[$v["e1"]]) && ($doc->autonext[$v["e1"]]==$v["e2"])) {
  	$tmain = sprintf('color="%s",style="setlinewidth(3)",arrowsize=1.0',
  		$style['autonext-color']
  	);
  }
  ...
  if ($label) { 
	...
	$line[]='"'.str_replace(" ","\\n",'act'.$k).'" [ label="'.$act.'", fixedsize=false,shape=box,color="'.$doc->getColor($v["e1"]).'" ];';
      ...
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="%s",decorate=false, color="%s", labelfontname=sans, label="%s"];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",'act'.$k),
		    $style['arrow-label-font-color'],
		    $style['arrow-color'],
		    "");
      $e1='act'.$k;
	} elseif ($tact[$v["e1"]]!="") {
    ...
    if ($m2) {
      ...
      $line[]=sprintf('"%s" -> "%s" [labelfontcolor="%s",decorate=false, color="%s", labelfontname=sans];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$m2.$k),
		    $style['arrow-label-font-color'],
		    $style['arrow-color']
    ...
    $line[]=sprintf('"%s" -> "%s" [labelfontcolor="%s",decorate=false, color="%s", labelfontname=sans,label="%s" %s];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$e2),
		    $style['arrow-label-font-color'],
		    $style['arrow-color'],
    ...
    $line[]=sprintf('"%s" -> "%s" [labelfontsize=6,color="%s" %s,labelfontname=sans, label="%s"];',
		    str_replace(" ","\\n",$e1),
		    str_replace(" ","\\n",$e2),
		    $style['arrow-color'],
		    $tmain,
