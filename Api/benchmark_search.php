...
function print_stat_search_fam(&$stat, &$fam, &$table) {
  echo sprintf($table['lineformat'],
	       ...
	       sprintf("%s%s", $fam['name'], (strlen($fam['inherit'])==0)?'':' *'),
	       $fam['id'],
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['search'][$fam['id']]['no_order']['time'],
	       $stat['search'][$fam['id']]['order_by_title']['time'],
	       $stat['search'][$fam['id']]['order_by_cdate']['time'],
	       $stat['search'][$fam['id']]['order_by_id']['time']
	       );
  ...
  if( $fam['fromid'] != 0 ) {
    echo sprintf($table['lineformat'],
		 $fam['name']." <>",
		 $fam['id'],
		 $stat['search'][$fam['id']]['only']['count'],
		 $stat['search'][$fam['id']]['profile'],
		 $stat['search'][$fam['id']]['only']['no_order']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_title']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_cdate']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_id']['time']
		 );
  }
}
...
function print_stat_search_fam_distinct(&$stat, &$fam, &$table) {
  echo sprintf($table['lineformat'],
	       ...
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['search'][$fam['id']]['no_order']['distinct']['time'],
	       $stat['search'][$fam['id']]['order_by_title']['distinct']['time'],
	       $stat['search'][$fam['id']]['order_by_cdate']['distinct']['time'],
	       $stat['search'][$fam['id']]['order_by_id']['distinct']['time']
	       );
  ...
  if( $fam['fromid'] != 0 ) {
    echo sprintf($table['lineformat'],
		 ...
		 $stat['search'][$fam['id']]['only']['count'],
		 $stat['search'][$fam['id']]['profile'],
		 $stat['search'][$fam['id']]['only']['no_order']['distinct']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_title']['distinct']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_cdate']['distinct']['time'],
		 $stat['search'][$fam['id']]['only']['order_by_id']['distinct']['time']
		 );
  }
}
...
function print_stat_regex_fam(&$stat, &$fam, &$table) {
  echo sprintf($table['lineformat'],
	       ...
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['regex'][$fam['id']]['no_order']['time'],
	       $stat['regex'][$fam['id']]['order_by_title']['time'],
	       $stat['regex'][$fam['id']]['order_by_cdate']['time'],
	       $stat['regex'][$fam['id']]['order_by_id']['time']
	       );
}
...
function print_stat_regex_fam_distinct(&$stat, &$fam, &$table) {
  echo sprintf($table['lineformat'],
	       ...
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['regex'][$fam['id']]['no_order']['distinct']['time'],
	       $stat['regex'][$fam['id']]['order_by_title']['distinct']['time'],
	       $stat['regex'][$fam['id']]['order_by_cdate']['distinct']['time'],
	       $stat['regex'][$fam['id']]['order_by_id']['distinct']['time']
	       );
}
...
function print_stat_fulltext_fam(&$stat, &$fam, $table) {
  echo sprintf($table['lineformat'],
	       ...
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['fulltext'][$fam['id']]['no_order']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_title']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_cdate']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_id']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_rank']['time']
	       );
}
...
function print_stat_fulltext_fam_distinct(&$stat, &$fam, $table) {
  echo sprintf($table['lineformat'],
	       ...
	       $stat['search'][$fam['id']]['count'],
	       $stat['search'][$fam['id']]['profile'],
	       $stat['fulltext'][$fam['id']]['no_order']['distinct']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_title']['distinct']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_cdate']['distinct']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_id']['distinct']['time'],
	       $stat['fulltext'][$fam['id']]['order_by_rank']['distinct']['time']
	       );
}
...
function bench_search_fam_no_order(&$stat, &$fam) {
  ...
  $stat['search'][$fam['id']]['no_order']['time'] = bench_searchdoc(array('fam' => $fam['id']));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['no_order']['time'] = bench_searchdoc(array('fam' => -$fam['id']));
  }

  $stat['search'][$fam['id']]['no_order']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'distinct' => true));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['no_order']['distinct']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'distinct' => true));
  }
}
...
function bench_search_fam_order_by_title(&$stat, &$fam) {
  ...
  $stat['search'][$fam['id']]['order_by_title']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'title'));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_title']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'title'));
  }

  $stat['search'][$fam['id']]['order_by_title']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'title', 'distinct' => true));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_title']['distinct']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'title', 'distinct' => true));
  }  
}
...
function bench_search_fam_order_by_cdate(&$stat, &$fam) {
  ...
  $stat['search'][$fam['id']]['order_by_cdate']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'cdate'));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_cdate']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'cdate'));
  }

  $stat['search'][$fam['id']]['order_by_cdate']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'cdate', 'distinct' => true));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_cdate']['distinct']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'cdate', 'distinct' => true));
  }
}
...
function bench_search_fam_order_by_id(&$stat, &$fam) {
  ...
  $stat['search'][$fam['id']]['order_by_id']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'id'));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_id']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'id'));
  }

  $stat['search'][$fam['id']]['order_by_id']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'], 'orderby' => 'id', 'distinct' => true));

  if( $fam['fromid'] != '0' ) {
    $stat['search'][$fam['id']]['only']['order_by_id']['distinct']['time'] = bench_searchdoc(array('fam' => -$fam['id'], 'orderby' => 'id', 'distinct' => true));
  }
}
...
function bench_regex_fam_no_order(&$stat, &$fam) {
  ...
  $stat['regex'][$fam['id']]['no_order']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									 'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word']))
								 ));

  $stat['regex'][$fam['id']]['no_order']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
										     'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word'])),
										     'distinct' => true
								 ));
}
...
function bench_regex_fam_order_by_title(&$stat, &$fam) {
  ...
  $stat['regex'][$fam['id']]['order_by_title']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									       'orderby' => 'title',
									       'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word']))
								 ));

  $stat['regex'][$fam['id']]['order_by_title']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											   'orderby' => 'title',
											   'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word'])),
											   'distinct' => true
								 ));
...
function bench_regex_fam_order_by_cdate(&$stat, &$fam) {
  ...
  $stat['regex'][$fam['id']]['order_by_cdate']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									 'orderby' => 'cdate',
									 'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word']))
								 ));

  $stat['regex'][$fam['id']]['order_by_cdate']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											   'orderby' => 'cdate',
											   'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word'])),
											   'distinct' => true
								 ));
...
function bench_regex_fam_order_by_id(&$stat, &$fam) {
  ...
  $stat['regex'][$fam['id']]['order_by_id']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'orderby' => 'id',
									    'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word']))
								 ));

  $stat['regex'][$fam['id']]['order_by_id']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											'orderby' => 'id',
											'filter' => sprintf("svalues ~* '%s'", pg_escape_string($parms['word'])),
											'distinct' => true
								 ));
...
function bench_fulltext_fam_no_order(&$stat, &$fam) {
  ...
  $stat['fulltext'][$fam['id']]['no_order']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'filter' => $ret_filter
									    )
								      );

  $stat['fulltext'][$fam['id']]['no_order']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											'filter' => $ret_filter,
											'distinct' => true
											)
										  );
...
function bench_fulltext_fam_order_by_title(&$stat, &$fam) {
  ...
  $stat['fulltext'][$fam['id']]['order_by_title']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'filter' => $ret_filter,
									    'orderby' => 'title'
									    )
								      );

  $stat['fulltext'][$fam['id']]['order_by_title']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											      'filter' => $ret_filter,
											      'orderby' => 'title',
											      'distinct' => true
											      )
...
function bench_fulltext_fam_order_by_cdate(&$stat, &$fam) {
  ...
  $stat['fulltext'][$fam['id']]['order_by_cdate']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'filter' => $ret_filter,
									    'orderby' => 'cdate'
									    )
								      );

  $stat['fulltext'][$fam['id']]['order_by_cdate']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											      'filter' => $ret_filter,
											      'orderby' => 'cdate',
											      'distinct' => true
											      )
...
function bench_fulltext_fam_order_by_id(&$stat, &$fam) {
  ...
  $stat['fulltext'][$fam['id']]['order_by_id']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'filter' => $ret_filter,
									    'orderby' => 'id'
									    )
								      );

  $stat['fulltext'][$fam['id']]['order_by_id']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											   'filter' => $ret_filter,
											   'orderby' => 'id',
											   'distinct' => true
											   )
...
function bench_fulltext_fam_order_by_rank(&$stat, &$fam) {
  ...
  $stat['fulltext'][$fam['id']]['order_by_rank']['time'] = bench_searchdoc(array('fam' => $fam['id'],
									    'filter' => $ret_filter,
									    'orderby' => $ret_orderby
									    )
								      );

  $stat['fulltext'][$fam['id']]['order_by_rank']['distinct']['time'] = bench_searchdoc(array('fam' => $fam['id'],
											     'filter' => $ret_filter,
											     'orderby' => $ret_orderby,
											     'distinct' => true
											     )
