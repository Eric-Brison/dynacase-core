...
$verbose = ('yes' === $usage->addOptionalParameter('verbose', 'verbose', array('yes', 'no'), 'no'));
$usage->verify();
...
class styleManager
{
        ...
        if (is_dir($customRulesDirPath) && is_readable($customRulesDirPath)) {
            ...
            if (false !== $customRulesFiles) {
                foreach ($customRulesFiles as $customRulesFile) {
                    $customRules = array_replace_recursive(
                        $customRules,
                        $this->loadCustomRulesFromFile($customRulesDirPath . DIRECTORY_SEPARATOR . $customRulesFile)
                    );
                }
            }
        }
    ...
    public function applyStyle()
    {
        ...
        // apply sty_rules
        $filesDefinition = $this->deployStyleFiles($styleConfig['sty_rules'], 'css');
        $style->setRules($filesDefinition);
        $err = $style->modify();
        if ($err) {
            throw new \Dcp\Style\Exception("STY0003", "error when modifying style");
        }
