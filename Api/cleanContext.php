...
$full = ($usage->addOptionalParameter("full", "full (yes or no)") == "yes");
...
if ($real || $full) system(sprintf("PGSERVICE=%s psql -a -f %s/API/cleanFullContext.sql | logger -t %s", escapeshellarg($dbfreedom) , escapeshellarg($dir) , escapeshellarg("cleanContext(" . $action->GetParam("CORE_CLIENT") . ")")));
else system(sprintf("PGSERVICE=%s psql -a -f %s/API/cleanContext.sql | logger -t %s", escapeshellarg($dbfreedom) , escapeshellarg($dir) , escapeshellarg("cleanContext(" . $action->GetParam("CORE_CLIENT") . ")")));
