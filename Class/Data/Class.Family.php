...
Class Fdl_Family extends Fdl_Document
{
    ...
    function addAttribute($id, $label, $order, $type, $parent = '', $visibility = 'W', $intitle = false, $inabstract = false, $needed = false, $link = '', $elink = '', $phpfile = "", $phpfunc = '', $constraint = '', $options = null)
    {
        if ($this->doc) {
            ...
            if ($this->completeAttributes($oattr, $id, $label, $order, $type, $parent, $visibility, $intitle, $inabstract, $needed, $link, $elink, $phpfile, $phpfunc, $constraint, $options)) {
                $err = $oattr->add();
    ...
    function modifyAttribute($id, $label, $order, $type, $parent = '', $visibility = 'W', $intitle = false, $inabstract = false, $needed = false, $link = '', $elink = '', $phpfile = "", $phpfunc = '', $constraint = '', $options = null)
    {
        if ($this->doc) {
            ...
            if (!$oattr->isAffected()) $oattr = new DocAttr($this->dbaccess, array(
                $oa->docid,
                ':' . $id
            ));
            if (!$oattr->isAffected()) {
            ...
            if ($this->completeAttributes($oattr, $oattr->id, $label, $order, $type, $parent, $visibility, $intitle, $inabstract, $needed, $link, $elink, $phpfile, $phpfunc, $constraint, $options)) {
                $err = $oattr->modify();
                if ($err) $this->setError($err);
                else {
                    $wsh = getWshCmd();
                ...
                if (preg_match("/^(upto|downto) ([a-z0-9_]+)$/", $order, $reg)) {
                    ...
                    if ($oar) {
                        ...
                        foreach ($needdown as $k => $v) {
                            $attr = new DocAttr($this->dbaccess, array(
                                $v->docid,
                                $v->id
                            ));
                            if (!$attr->isAffected()) $attr = new DocAttr($this->dbaccess, array(
                                $v->docid,
                                ':' . $v->id
                            ));
                            if ($attr->isAffected()) {
