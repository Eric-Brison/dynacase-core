...
msgstr "%s"',$key,str_replace('"','\"',$text));
	...
	$cmd=sprintf('cd %s/locale/%s/LC_MESSAGES;msgunfmt 0w.mo > _w.po && msgcat --use-first  %s _w.po > __w.po && msgfmt __w.po -o 0w.mo && %s/whattext',
		     DEFAULT_PUBDIR,$lang,$mofile,DEFAULT_PUBDIR);
      } else {
	$cmd=sprintf('cd %s/locale/%s/LC_MESSAGES;msgfmt %s  -o 0w.mo && %s/whattext',
		     DEFAULT_PUBDIR,$lang,$mofile,DEFAULT_PUBDIR);
      }
