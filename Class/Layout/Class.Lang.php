<?php
// ---------------------------------------------------------------
// $Id: Class.Lang.php,v 1.1 2002/01/08 12:41:34 eric Exp $
...
class Lang extends DbObj {
  ...
  var $fields = array("idapp", "lang", "code", "fmt");
  var $id_fields = array("idapp", "lang", "code");
  var $dbtable = "lang";
...
';
  ...
  function Exist($idapp, $code, $lang) {
    ...
    $query->basic_elem->sup_where = array("idapp={$idapp}", "lang='{$lang}'", "code='{$code}'");
    $query->Query();
  ...
  function SetEnv($id,$lang,$deflang) {
    ...
    $this->deflang = $lang;
    $query = new QueryDb($this->dbaccess, "Lang");
    $query->basic_elem->sup_where = array("idapp={$id}", "lang='{$deflang}'");
    $lista = $query->Query();
    $query->basic_elem->sup_where = array("idapp={$id}", "lang='{$lang}'");
    $listb = $query->Query();
  ...
  function deletecatalog($idapp) {
    $query = new QueryDb($this->dbaccess, "Lang");
    $query->basic_elem->sup_where = array("idapp={$idapp}");
    $list = $query->Query(); 
    if ($query->nb>0) {
