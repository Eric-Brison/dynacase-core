...
Class Dir extends PDir
{
	...
	function AddFile($docid, $mode="latest",$noprepost=false,$forcerestrict=false,$nocontrol=false) {
		...
		if (! $qf->Exists()) {
			...
			if ($err == "") {
				...
				$this->addLog('addcontent',array("insert"=>array("id"=>$doc->id,"title"=>$doc->title)));
	...
	*/
			...
			if (! $this->isAuthorized($tdoc["fromid"])) {
				...
				if ($err == "") {
					...
					if ($err == "") {
						...
						$this->addLog('addcontent',array("insert"=>array("id"=>$tdoc["id"],"title"=>$tdoc["title"])));
						$tAddeddocids[]=$docid;
	...
	function DelFile($docid,$noprepost=false, $nocontrol=false ) {
		...
		$this->addLog('delcontent',array("insert"=>array("id"=>$doc->id,"title"=>$doc->title)));
	      	...
	      	if (method_exists($this,"delFile")) {
	      		...
	      		if ($err=="") {
	      			...
	      			if ($doc->isAlive()) {
	      				...
	      				$err=$doc->modify(true,array("prelid"),true);
	      			}
	      		}
