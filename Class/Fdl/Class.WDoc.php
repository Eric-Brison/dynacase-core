...
Class WDoc extends Doc {
  ...
  function createProfileAttribute() {
    ...
    $this->exec_query(sprintf("delete from docattr where docid=%d  and options ~ 'autocreated=yes'",
			      intval($cid)));
    $this->getStates();
  ...
  function ChangeState ($newstate, $addcomment="", $force=false,$withcontrol=true,$wm1=true,$wm2=true,$wneed=true) {
    ...
    if ($wm1 && ($tr["m1"] != ""))  {
      ...
      $err = call_user_func(array($this, $tr["m1"]), $newstate, $this->doc->state, $addcomment);
    ...
    if ($wm2 && ($tr["m2"] != "")) {
      ...
      $err = call_user_func(array($this, $tr["m2"]), $newstate, $oldstate, $addcomment);
