...
create unique index i_docir on doc(initid, revision);";
    ...
    final public function CanLockFile()
    {
        ...
        if ($this->locked == - 1) {
            ...
            $err = sprintf(_("cannot lock document %s [%d] (rev %d) : fixed. Get the latest version") , $this->title, $this->id, $this->revision);
        } else {
            ...
            // test if is not already locked
                ...
                if (abs($this->locked) != $this->userid) {
                    $user = new User("", abs($this->locked));
                    if ($this->locked < - 1) $err = sprintf(_("Document %s is in edition by %s.") , $this->getTitle() , $user->firstname . " " . $user->lastname);
                    else $err = sprintf(_("cannot lock file %s [%d] : already locked by %s.") , $this->title, $this->id, $user->firstname . " " . $user->lastname);
                } else {
                    $err = $this->Control("edit");
                }
            }
    ...
    public function GetSortAttributes()
    {
        ...
        foreach ($nattr as $k => $a) {
            if ($a->getOption('sortable') != 'yes') {
                if ($a->repeat || ($a->visibility == "H") || ($a->visibility == "I") || ($a->visibility == "O") || ($a->type == "longtext") || ($a->type == "xml") || (($a->type == "docid") && ($a->getOption("doctitle") == "")) || ($a->type == "htmltext") || ($a->type == "image") || ($a->type == "file") || ($a->fieldSet->visibility == "H") || ($a->getOption('sortable') == 'no')) continue;
            }
            $tsa[$a->id] = $a;
        }
        return $tsa;
                    ...
                    function viewbodycard($target = "_self", $ulink = true, $abstract = false, $onlyopt = false)
                    {
                        ...
                        function viewproperties($target = "finfo", $ulink = true, $abstract = true)
                        {
                            ...
                            if ((abs($this->profid) > 0) && ($this->profid != $this->id)) {
                                ...
                                if ($this->profid == 0) $this->lay->Set("profile", _("no access control"));
                                else {
