...
// ---------------------------------------------------------------------------
//  $Id: Class.Form.php,v 1.1 2002/01/08 12:41:34 eric Exp $
...
Class Form {
  ...
  function CellTextArea( $CREATION,$libelle,$rows=5,$cols=55,$name,$entite,$span=1) {
    if (!$CREATION) { $value=$entite->$name;} else {$value='';};
    $src="\t<TD CLASS=\"{$this->label_class}\" COLSPAN=$span>$libelle\n";
    $src.="<DIV CLASS=\"finput\"><FONT SIZE=\"-1\">".
  ...
  function CellInputId( $CREATION,$libelle,$max,$size,$name,$id,$entite,$span=1) {
    if (!$CREATION) { $value=$entite->$id;} else {$value='';};
    $src="\t<TD CLASS=\"{$this->label_class}\" COLSPAN=$span>$libelle\n";
    $src.="<FONT SIZE=\"-1\">".
          "<INPUT MAXLENGTH=$max SIZE=$size NAME=\"p_$name\"";
    if ($CREATION) {
