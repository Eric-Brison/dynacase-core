    ...
    */
            ...
            if (isset($file["props"]) && is_array($file["props"])) {
                ...
                foreach ($file["props"] as $key => $prop) {
                    ...
                    if (!isset($prop["val"]) || $prop["val"] === "" || $prop["val"] === false) {
                        ...
                        switch ($prop["name"]) {
                            case "creationdate":
                                echo "     <D:creationdate ns0:dt=\"dateTime.tz\">" . gmdate("Y-m-d\\TH:i:s\\Z", $prop['val']) . "</D:creationdate>\n";
                                break;

                            case "getlastmodified":
                                echo "     <D:getlastmodified ns0:dt=\"dateTime.rfc1123\">" . gmdate("D, d M Y H:i:s ", $prop['val']) . "GMT</D:getlastmodified>\n";
                                break;
                            ...
                            default:
                                echo "     <D:$prop[name]>" . $this->_prop_encode(htmlspecialchars($prop['val'])) . "</D:$prop[name]>\n";
                                break;
                        }
                    } else {
                        // properties from namespaces != "DAV:" or without any namespace
                        if ($prop["ns"]) {
                            echo "     <" . $ns_hash[$prop["ns"]] . ":$prop[name]>" . $this->_prop_encode(htmlspecialchars($prop['val'])) . "</" . $ns_hash[$prop["ns"]] . ":$prop[name]>\n";
                        } else {
                            echo "     <$prop[name] xmlns=\"\">" . $this->_prop_encode(htmlspecialchars($prop['val'])) . "</$prop[name]>\n";
                        }
                    }
                }
    ...
    function http_PROPPATCH()
    {
        if ($this->_check_lock_status($this->path)) {
            ...
            if ($responsedescr) {
                echo "  <D:responsedescription>" . $this->_prop_encode(htmlspecialchars($responsedescr)) . "</D:responsedescription>\n";
            }
    ...
    function _copymove($what)
    {
        ...
        /*	!strncmp($_SERVER["SCRIPT_NAME"], $path,
         strlen($this->__SERVER["SCRIPT_NAME"]))*/) {
            //			$options["dest"] = substr($path, strlen($this->__SERVER["SCRIPT_NAME"]));
    ...
    function _allow()
    {
        ...
        if (isset($allow["GET"])) $allow["HEAD"] = "HEAD";
    ...
    function _check_auth()
    {
        if (method_exists($this, "checkAuth")) {
            ...
            return $this->checkAuth(@$this->_SERVER["AUTH_TYPE"], @$this->_SERVER["PHP_AUTH_USER"], @$this->_SERVER["PHP_AUTH_PW"]);
        } else if (method_exists($this, "check_auth")) {
            // old (pre 1.0) method name
            return $this->check_auth(@$this->_SERVER["AUTH_TYPE"], @$this->_SERVER["PHP_AUTH_USER"], @$this->_SERVER["PHP_AUTH_PW"]);
        } else {
        ...
        if (method_exists($this, "checkLock")) {
            ...
            if (is_array($lock) && count($lock)) {
                ...
                if (!isset($this->_SERVER["HTTP_IF"]) || !strstr($this->_SERVER["HTTP_IF"], $lock["token"])) {
                    if (!$exclusive_only || ($lock["scope"] !== "shared")) return false;
                }
            }
        }
        return true;
