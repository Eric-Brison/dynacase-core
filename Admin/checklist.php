...
$tout["main connection db"] = array(
    "status" => $dbr_anakeen ? OK : KO,
    "msg" => $pgservice_core
);
...
if ($dbr_anakeen) {
    ...
    $tout["unreference user in group"] = array(
        "status" => (count($pout) == 0) ? OK : BOF,
        "msg" => $msg
    );
    ...
    $tout["user as group"] = array(
        "status" => (count($pout) == 0) ? OK : KO,
        "msg" => $msg
    );
    ...
    $tout["unreference actions"] = array(
        "status" => (count($pout) == 0) ? OK : BOF,
        "msg" => $msg
    );
    ...
    $tout["unreference parameters"] = array(
        "status" => (count($pout) == 0) ? OK : BOF,
        "msg" => $msg
    );
    ...
    $tout["unreference permission"] = array(
        "status" => ($nb == 0) ? OK : BOF,
        "msg" => $msg
    );
    ...
    $tout["connection db dynacase"] = array(
        "status" => $dbr_freedom ? OK : KO,
        "msg" => $fdb
    );
    ...
    if ($rf) {
        ...
        while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)) {
            ...
            if ($fromid == 0) $fromid = "";
            $fid = intval($row["id"]);
            $test = pg_query($rf, sprintf("SELECT relname from pg_class where oid in (SELECT inhparent from pg_inherits where inhrelid =(SELECT oid FROM pg_class where relname='doc%d'));", $fid));
    ...
    if ($fdb) {
        ...
        $tout["connection db webdav"] = array(
            "status" => $dbr_webdav ? OK : KO,
            "msg" => $fdb
        );
    }
    ...
    if ($nuAppExists && $ldaphost) {
        ...
        array_push($baseList, array(
            'dn' => $GP['NU_LDAP_USER_BASE_DN'],
            'filter' => $GP['NU_LDAP_USER_FILTER']
        ));
        array_push($baseList, array(
            'dn' => $GP['NU_LDAP_GROUP_BASE_DN'],
            'filter' => $GP['NU_LDAP_GROUP_FILTER']
        ));
