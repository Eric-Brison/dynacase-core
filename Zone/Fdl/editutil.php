...
function getHtmlInput(&$doc, &$oattr, $value, $index="",$jsevent="",$notd=false) {
  ...
  case "xml":     
    ...
    $input="<textarea $oc wrap=\"virtual\"  onkeyup=\"textautovsize(event,this)\"  onclick=\"textautovsize(event,this)\" class=\"autoresize\" style=\"height:$lh\" name=\"".
      $attrin."\" ";
    $input .= " id=\"".$attridk."\" "; 
    if (($visibility == "R")||($visibility == "S")) $input .=$idisabled;
    $input .= " >".
  ...
  case "thesaurus": 
	...
	foreach ($thids as $kth=>$vth) {	
	  ...
	  if ($th->isAlive()) {
	    ...
	    $top[]=array("ltitle"=>substr($thtitle,0,100),
			 "ldocid"=>$vth);
	  }
	}
  ...
  case "date": 
    ...
    if (($doc->usefor != 'D') && ($oattr->usefor!='Q'))	$lay->setBlockData("CONTROLCAL",array(array("zou")));
    $input =trim($lay->gen()); 
    break;     
    //----------------------------------------
  ...
  default : 
	...
	"</textarea>";
      ...
      $hvalue=str_replace(array("[","$"),array("&#091;","&#036;"),chop(htmlentities(stripslashes($value),ENT_COMPAT,"UTF-8")));
  ...
  if (($oattr->type != "array") && ($oattr->type != "htmltext")) {
	...
	if (preg_match('/[A-Z_\-0-9]+:[A-Z_\-0-9]+\(/i',$phpfunc)) {
        $input.="<input id=\"ic_${linkprefix}$attridk\" type=\"button\" value=\"Z\"".
          " title=\"".$ititle."\"".
	...
	//$input.="</td><td>";    
      } else  if (preg_match("/(.*)\((.*)\)\:(.*)/", $phpfunc, $reg)) {
	  if ($alone) {
	    $arg = array($oattr->id);
	  } else {
	...
	$input.="<input style=\"background-color:$color;\"type=\"button\" class=\"constraint\" id=\"co_$attridk\" value=\"C\"".
...
function getLayArray(&$lay,&$doc,&$oattr,$row=-1) {
  ...
  $lay->set("thspan","2");
	...
	$talabel[] = array("alabel"=>(!$visible)?"":$v->getLabel(),
	...
	$tilabel[] = array("ilabel"=>getHtmlInput($doc,$v,$ddoc->getValue($tad[$k]->id),'__1x_'),
	...
	$pindex='s';
      ...
      $tvattr = array();
      for ($k=0;$k<$nbitem;$k++) {	if (($row>=0) && ($k!=$row)) continue;
	...
	while (list($ka, $va) = each($ta)) {
	  ...
	  $tivalue[]=array("eivalue"=>getHtmlInput($doc,$va,$tval[$ka][$k],$pindex.$k),
...
function getZoneLayArray(&$lay,&$doc,&$oattr,$zone) {
  ...
  foreach ($tr as $kd=>$td) {
    $dval = preg_replace("/\[([^\]]*)\]/e",
			 "rowattrReplace(\$doc,'\\1','__1x_',\$defval)",
			 $td);
