		    ...
		    $famid="")       // folder containt special fam id 
{
  ...
  if (is_array($ldoc)) {
    ...
    $doc = createTmpDoc($dbaccess,$famid);
	...
	if ($zdoc["fromid"]!=$doc->fromid) {
	  if (!isset($tcdoc[$zdoc["fromid"]]))  {
	    $tcdoc[$zdoc["fromid"]]=createTmpDoc($dbaccess,$zdoc["fromid"]);	 
	  }
	...
	if ($doc->isConfidential()) {
	  ...
	  foreach($lattr as $ka=>$attr)  {	
	    ...
	    else  $tvalues[]=$doc->getHtmlValue($attr,$doc->getValue($attr->id,"-"),$target,true,-1,true,true);
	  }
	}
